...
static msg::LocalizationEstimateMsg make_loc(double vx, double cov_xy=0.0)
{
  ...
  for(int i=0;i<6;i++) for(int j=0;j<6;j++) m.est_pose.covariance(i,j)=0.0;
...
static msg::LidarDataMsg make_scan(const std::vector<double>& ranges, double range_min=0.1, double range_max=10.0)
{
  msg::LidarDataMsg m{};
