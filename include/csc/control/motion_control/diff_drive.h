 ...
 * of the diff drive functionality is determined at compile-time using conditional compilation (`USE_SIM` flag),
 * allowing for flexibility in selecting between different control targets (e.g., hardware-based or Gazebo simulation).
 */
#pragma once
 ...
 *
 * @note The specific implementation (e.g., `gazebo_diff_drive.cpp` for simulation or a hardware-specific file)
 *       is chosen at compile time based on the `USE_SIM` CMake option.
 */
class DiffDrive
{
public:
   ...
   *
   * @note Since this is an abstract interface, the constructor does not initialize any hardware or simulation.
   ...
   *
   * @note This function initiates the control of the differential drive. The actual behavior of starting
   ...
   *
   * @note This function halts differential drive control. The behavior of stopping (e.g., disabling hardware,
   ...
   *
   * @note This function sends velocity commands to the diff drive. The actual command sending mechanism
