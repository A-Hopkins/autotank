...
void Odom::start(std::function<void(const msg::OdomDataMsg&)> callback)
{
  ...
  node.Subscribe<gz::msgs::Odometry>(
      "/odom",
      [this](const gz::msgs::Odometry& msg)
      {
        ...
        msg::OdomDataMsg odom_data = {
            ...
            // Populate pose data.
            .pose = {.pose = {.point       = {msg.pose().position().x(), msg.pose().position().y(),
                                              msg.pose().position().z()},
            ...
            // Populate twist data.
            .twist = {.twist = {.linear  = {msg.twist().linear().x(), msg.twist().linear().y(),
                                            msg.twist().linear().z()},
                                .angular = {msg.twist().angular().x(), msg.twist().angular().y(),
                                            msg.twist().angular().z()}},
